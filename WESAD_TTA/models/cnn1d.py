...
class StressCNN1D(nn.Module):
    ...
    入力形状は `(batch, channels, time)`。出力は `CrossEntropyLoss` に渡す
    3 クラス logits `(batch, num_classes)` で、softmax は損失関数や評価関数側で
    適用する。
    """
    ...
    def classify_features(self, features):
        """64 次元 feature からクラス logits を計算する。"""
        features = self.classifier[5](features)
        return self.classifier[6](features)
